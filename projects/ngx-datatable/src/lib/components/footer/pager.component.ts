  ...
  template: `
    <ul class="pager">
      <li [class.disabled]="!canPrevious()">
        <a role="button" [attr.aria-label]="messages.ariaFirstPageMessage ?? 'go to first page'" (click)="selectPage(1)">
          <i class="{{ pagerPreviousIcon ?? 'datatable-icon-prev' }}"></i>
        </a>
      </li>
      <li [class.disabled]="!canPrevious()">
        <a role="button" [attr.aria-label]="messages.ariaPreviousPageMessage ?? 'go to previous page'" (click)="prevPage()">
          <i class="{{ pagerLeftArrowIcon ?? 'datatable-icon-left' }}"></i>
        </a>
      </li>
      @for (pg of pages; track pg.number) {
        <li
          [attr.aria-label]="(messages.ariaPageMessage ?? 'page') + ' ' + pg.number"
          class="pages"
      ...
      <li [class.disabled]="!canNext()">
        <a role="button" [attr.aria-label]="messages.ariaNextPageMessage ?? 'go to next page'" (click)="nextPage()">
          <i class="{{ pagerRightArrowIcon ?? 'datatable-icon-right' }}"></i>
        </a>
      </li>
      <li [class.disabled]="!canNext()">
        <a role="button" [attr.aria-label]="messages.ariaLastPageMessage ?? 'go to last page'" (click)="selectPage(totalPages)">
