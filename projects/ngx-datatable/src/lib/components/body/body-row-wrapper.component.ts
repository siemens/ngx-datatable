  ...
  implements DoCheck, OnInit, OnChanges
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['rowIndex']) {
      if (this.rowContext) {
        this.rowContext.rowIndex = this.rowIndex ?? 0;
      } else if (this.groupContext) {
        this.groupContext.rowIndex = this.rowIndex ?? 0;
      }
    }
    if (changes['expanded']) {
  ...
  ngDoCheck(): void {
    ...
    if (this.groupHeader?.checkboxable && this.selectedRowsDiffer.diff(this.selected)) {
      const selectedRows = this.selected.filter(row =>
        this.group()?.value.find((item: any) => item === row)
      );
  ...
  onCheckboxChange(groupSelected: boolean): void {
    ...
    this.selected = [...this.selected.filter(row => !groupValue.find((item: any) => item === row))];
    // If checkbox is checked then add all rows of this group in `selected`
    if (groupSelected) {
      this.selected = [...this.selected, ...groupValue];
    }
